...
export default function CategoriasTable({ categorias, onEdit, onDelete,onSaved }: Props) {
  const [filter, setFilter] = useState("");
  const [pageSize, setPageSize] = useState(5);
  const [pageIndex, setPageIndex] = useState(0);
  const [isOpen, setIsOpen] = useState(false);
 
   const [categoriaEdit, setCategoriaEdit] = useState<any | null>(null);
  ...
  const columns = useMemo<ColumnDef<Categoria>[]>(
        () => [
            ...
            cell: ({ row }) => {
                ...
                <div className="flex justify-center gap-2">
                    ...
                    onClick={() => {
                        setCategoriaEdit(item);   // ← tipo correcto Categoria
                        setIsOpen(true);
                    }}
                    >
                    <Pencil size={16} />
        ...
        [onDelete, setCategoriaEdit, setIsOpen]
        );
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        >  <div className="flex items-center gap-2">
         ...
         <Modal 
            ...
            onClose={() => {
              setIsOpen(false);
              setCategoriaEdit(null);   // 👈 Ahora sí se limpia              
            }}
          >
